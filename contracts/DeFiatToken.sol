...
contract DeFiatToken is ERC20("DeFiat", "DFT") {  //overrides the _transfer function and adds burn capabilities
...
//== mastermind ==
    ...
    function updateTxStruct(address sender, address recipient) internal returns(bool){
        ...
        transaction.actualDiscount = Math.max(transaction.senderDiscount, transaction.recipientDiscount);
    ...
    //Calculates burn and fees on discounted amount (burn and fees are 0.0X% ie base 10000)
        ...
        if(_toFee > 0) {
            ERC20._transfer(sender, transaction.feeDestination, _toFee); //native _transfer + emit
        } //transfer fee
        
        if(_toBurn > 0) {
            ERC20._burn(sender,_toBurn);
